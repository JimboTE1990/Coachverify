...
export const TrialLoginNotification: React.FC<TrialLoginNotificationProps> = ({ coach }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 max-w-md animate-slide-in-right">
      <div className="bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 border-2 border-amber-300 rounded-2xl shadow-2xl p-6 relative">
        {/* Dismiss Button */}
        <button
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-black text-slate-900">You're on a Free Trial</h3>
            <div className="flex items-center gap-1.5 text-sm text-amber-700 font-bold">
        ...
        <Link
          to="/pricing"
          onClick={handleDismiss}
          className="block w-full text-center bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 text-white font-black py-3 px-4 rounded-xl hover:from-amber-600 hover:via-orange-600 hover:to-rose-600 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
        >
          Upgrade to Premium Now →
        </Link>

        <p className="text-xs text-center text-slate-500 mt-3">
          From £15/month • Lock in your rate today
        </p>
      </div>
    </div>
  );
