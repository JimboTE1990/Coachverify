...
serve(async (req) => {
  ...
  try {
    ...
    // When EMCC directory is restored, remove this block and uncomment the code below
    console.log('[EMCC URL Verification] TEMPORARY: Auto-approving for manual review (directory unavailable)');

    return new Response(JSON.stringify({
      verified: false,
      pendingManualReview: true,
      confidence: 0,
      reason: 'Due to EMCC recently updating their website, directories are temporarily unavailable. We are manually verifying all EMCC certificates. Please email your EMCC certificate to coachdogverify@gmail.com with your EIA number in the subject line.'
    }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });

    /* ORIGINAL CODE - Restore when EMCC directory is available
 ...
 * TEMPORARY: Also accepts plain EIA numbers due to EMCC directory unavailability
 */
function validateEMCCUrl(url: string): { valid: boolean; eiaNumber?: string; reason?: string } {
  // TEMPORARY: Check if input is a plain EIA number (not a URL)
  const eiaPattern = /^EIA\d+$/i;
  if (eiaPattern.test(url.trim())) {
    console.log('[EMCC Validation] Detected plain EIA number format (temporary mode)');
    return {
      valid: true,
      eiaNumber: url.trim().toUpperCase()
    };
  }

  // Original URL validation logic
  try {
    ...
    return {
      valid: false,
      reason: 'Invalid format. Please enter your EIA number (e.g., EIA20230480) or the complete URL from the EMCC directory.'
    };
  }
}
 ...
 * TEMPORARY: Returns pendingManualReview if directory is unavailable
 */
async function verifyEMCCUrlContent(
  ...
  try {
    ...
    // TEMPORARY: If URL is not a real URL (just EIA number), construct placeholder URL
    let fetchUrl = url;
    if (!url.startsWith('http')) {
      fetchUrl = `https://www.emccglobal.org/accreditation/eia/eia-awards/?reference=${expectedEIA}&search=1`;
      console.log('[EMCC Content Verification] Constructed URL from EIA number:', fetchUrl);
    }
    ...
    if (!response.ok) {
      // TEMPORARY: Directory unavailable - trigger manual review
      console.log('[EMCC Content Verification] Directory unavailable (HTTP error), triggering manual review');
      return {
        verified: false,
        pendingManualReview: true,
        confidence: 0,
    ...
    // TEMPORARY: Check if directory page is unavailable or showing errors
    if (html.length < 500 || html.includes('404') || html.includes('Not Found') || !html.includes('EMCC')) {
      console.log('[EMCC Content Verification] Directory appears unavailable or empty, triggering manual review');
      return {
        verified: false,
        pendingManualReview: true,
        confidence: 0,
